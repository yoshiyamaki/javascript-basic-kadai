...
const countBtn = document.getElementById('count-btn'); // count-btnというidを持つHTML要素を取得する

countBtn.addEventListener('click', () => { // HTML要素がクリックされたときにイベント処理を実行する
  const text = document.forms.textForm.textBox.value; // テキストボックスに入力された文字列を取得する

  const count = text.length; // 取得した文字列の文字数をカウントする
  console.log(`文字数: ${count}`); // 文字数をコンソールに出力する
});
